(function(){
	...
	ink.vis.timestack = function(){
		...
		timestack.on = function(type, listener) {
			event.on(type,listener)//[type].add(listener)
			return timestack;
		}
		...
		timestack.update = function(){
			...
			var w = width || parseInt(d3.select(target).style("width")),
			    h = height || parseInt(d3.select(target).style("width")),
			    p = [20, 20, 20, 20],
		  ...
		  var rect = cause.selectAll("rect")
		    ...
		    .enter().append("svg:rect")
			  ...
			  .attr("data-content", function(d){
				  var s = "<b>Year: </b>" + d.x.getFullYear() + "<br/><b>Letters: </b>" + d.y;
				  return s;
			  })
			rect.append("svg:title")
				.text(function(d,i) { return d.d + " (" + (d.y/stack[dimension][0][i].y*100).toFixed(2) + "%)"; });
		  ...
		  var label = vis.selectAll("text")
		      ...
		      .attr("dy", ".71em")
			  .on("click",function(d){ event.labelclick(d)})
		      .text(format);
...
//-				.attr("transform", function(d) { return "translate(0,-50)"; });
	...
	ink.vis.map = function(){
		...
		var map = {},
			...
			vis,
			po = org.polymaps,
			cloudmadeURL = "http://tile.cloudmade.com/1a1b06b230af4efdbb989ea99e9841af/6083/256/{Z}/{X}/{Y}.png",
			line = d3.svg.line()
				.interpolate('bundle')		
		...
		map.update = function(){
			...
			width = width || parseInt(d3.select(target).style("width")),
